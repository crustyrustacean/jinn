...
- (slices) Slice code lives in per-slice crates (`jinn-dashboard`, `jinn-quake-bar`, `jinn-discord-slice`; EXPORT slices add a `jinn-<slice>-msg` contracts crate, plus `jinn-session-msg` — the session family's crossing-contract home, holding the discriminant and lifecycle events slices consume); the kernel references a slice only through its `activate()` call.
