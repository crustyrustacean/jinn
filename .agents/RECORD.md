...
- (theme) Theme discovery flows through a user-installed themes plugin (source-built, `jinn plugin install`): it scans `~/.config/jinn/themes/*.toml` (ANSI name, ANSI code, hex, RGB formats) and contributes full theme definitions over the plugin wire; the theme picker reads the contribution cache, not disk. A default install ships no plugins and uses the embedded default theme.
