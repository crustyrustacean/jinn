...
- (session) A session optionally carries a project association (a directory path) stamped only when created from the projects UI; it persists in the session metadata blob, is inherited by forks and subagents, and never follows cwd changes.
