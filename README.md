...
- [`providers.toml`](./crates/jinn-provider-config/src/default_providers.toml) - LLM provider configuration. Per-model metadata (context window, input modalities, request body params) can be set with `[[providers.model_info]]` tables; hand-authored values take precedence over API-discovered data. Create a new one with `jinn config providers`.
- `themes/` - color themes
- `personas/` - personas
