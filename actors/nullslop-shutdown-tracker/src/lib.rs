//! Shutdown tracker actor — manages actor lifecycle and shutdown coordination.
//!
//! Subscribes to `ActorStarting`, `ActorStarted`, `ActorShutdownCompleted` events
//! and the `ProceedWithShutdown` command. Tracks which actors are running and
//! emits `ProceedWithShutdown` when all actors have completed shutdown.
...
impl ShutdownTrackerActor {
    ...
    fn handle_event(&mut self, event: &Event, ctx: &ActorContext) {
        match event {
            ...
            Event::ActorStarted { .. } => {
                // Informational only — tracker already has the name from ActorStarting.
            }
