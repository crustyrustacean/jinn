    ...
    if let Some(workflow) = state.workflow.active() {
            ...
            && let Some(outputs) = &node_state.outputs
        {
            ...
            for (_, value) in outputs.iter() {
                if let jinn_workflow::port::PortValue::Single(
                    jinn_workflow::port::ScalarValue::Text(s),
                ) = value
                {
                    state
                        .frontend
...
pub fn handle_workflow_input_submit(state: &mut AppState) -> IntentResult {
        ...
        .and_then(|ports| {
            ports
                ...
                .find(|p| {
                    matches!(
                        p.value_type,
                        jinn_workflow::port::PortType::Single(
                            jinn_workflow::port::ScalarType::Text
                        )
                    )
                })
                .map(|p| p.name.clone())
    ...
    workflow
        .execution
        .set_status(&node_name, jinn_workflow::engine::NodeStatus::Pending);
