...
pub fn get_global_data<T: DeserializeOwned + Default>(key: &str) -> Option<T> {
    crate::jinn::plugin::host::get_global_data(key)
...
impl Keybind {
    #[must_use]
    pub fn new(scope: impl Into<String>, keys: impl Into<String>, action: impl Into<String>) -> Self {
        Self {
            scope: scope.into(),
            keys: keys.into(),
            action: action.into(),
...
impl Tool {
    ...
    pub fn into_wit(self) -> WitToolDecl {
        WitToolDecl {
            name: self.name,
            description: self.description,
            parameters: self.parameters.into_iter().map(ToolParam::into_wit).collect(),
...
impl Segment {
    ...
    pub fn muted(self) -> Self {
        Self { style: Style::MutedText, ..self }
    }

    pub fn streaming(self) -> Self {
        Self { style: Style::Streaming, ..self }
    }
...
impl BadgeDirective {
    ...
    #[must_use]
    pub fn input_badge(segments: impl IntoIterator<Item = Segment>) -> crate::jinn::plugin::types::BadgeDirective {
...
pub trait Plugin: Sized {
    ...
    async fn run_tool(_name: String, _args: String, _ctx: crate::jinn::plugin::types::ToolCtx) {}
    ...
    ($ty:ident) => {
        impl crate::exports::jinn::plugin::hooks::Guest for $ty {
            ...
            async fn run_trigger(
                action: String,
                ctx: crate::jinn::plugin::types::TriggerCtx,
            ) {
