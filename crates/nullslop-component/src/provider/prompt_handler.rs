...
impl PromptAssemblyHandler {
    ...
    fn on_strategy_state_updated(
        ...
        ctx.state
            .session_mut(&evt.session_id)
            .set_strategy_state(evt.blob.clone());
    }
}
...
mod tests {
    ...
    fn strategy_state_updated_stores_blob() {
        ...
        // Then the blob is stored on the session.
        let blob = state.active_session().strategy_state();
        assert!(blob.is_some());
    ...
    fn strategy_state_updated_overwrites_existing() {
        ...
        // When processing two StrategyStateUpdated events.
        for count in [1, 2] {
            bus.submit_event(Event::StrategyStateUpdated {
                payload: StrategyStateUpdated {
                    session_id: session_id.clone(),
        ...
        let blob = state.active_session().strategy_state().unwrap();
        assert_eq!(blob["compaction_count"], 2);
    }
}
