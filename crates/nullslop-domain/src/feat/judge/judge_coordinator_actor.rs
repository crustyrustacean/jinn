...
mod tests {
    ...
    // --- Phase 7: Judge triggering during compaction ---

    #[rstest::rstest]
    #[tokio::test]
    async fn judge_not_triggered_on_compacting_phase() {
        // Given an origin session with an attached judge.
        let state = State::new(AppState::default());
        let (origin_id, origin) = make_origin_session();
        state.write().session.insert(origin);
    ...
    #[rstest::rstest]
    #[tokio::test]
    async fn judge_not_triggered_on_sending_phase() {
        // Given an origin session with an attached judge.
        let state = State::new(AppState::default());
        let (origin_id, origin) = make_origin_session();
        state.write().session.insert(origin);
        ...
        // When origin transitions to Sending (NOT Idle).
