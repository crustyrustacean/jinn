...
fn collect_bindings_for_scope_separates_scopes() {
    ...
    // When collecting bindings for Input scope.
    let entries = collect_bindings_for_scope(&keymap, &Scope::Input, &default_theme());

    // Then Input scope has bindings (scope isolation).
    assert!(!entries.is_empty(), "Input scope should have bindings");
    // And all entries are Input-scope only.
    for entry in &entries {
        assert_eq!(
            entry.scope, "Input",
            "all entries should be Input scope, got: {}",
            entry.scope
        );
    }
}
...
fn bindings_include_normal_scope() {
    ...
    // Then entries from Normal scope are present.
