...
mod tests {
    ...
    fn step_output_def_file_roundtrips() {
        ...
        let back: StepOutputDef = serde_json::from_str(&json).unwrap();
    ...
    fn step_def_roundtrips() {
        let step = StepDef {
            ...
            instructions: "Ask the user...".to_owned(),
            model_hint: ModelHint::Small,
            checkpoint: true,
            requires_user_input: true,
            tools: vec!["shell".to_owned(), "file_read".to_owned()],
            guards: GuardExpr::All {
                all: vec![GuardExpr::Predicate(
                    crate::guard::GuardPredicate::FileExists {
                        path: "{{video_dir}}/notes.md".to_owned(),
                    },
                )],
            },
            outputs: vec![StepOutputDef::File {
                label: "Video description".to_owned(),
                path: "{{video_dir}}/notes.md".to_owned(),
            }],
            depends_on: vec![],
    ...
    fn workflow_def_roundtrips() {
        let def = WorkflowDef {
            ...
            model_overrides: HashMap::from([("small".to_owned(), "ollama/phi3".to_owned())]),
            globals: HashMap::from([("base_dir".to_owned(), "/tmp".to_owned())]),
            steps: vec![StepDef {
                id: "step-1".to_owned(),
        ...
        assert_eq!(def.version, back.version);
