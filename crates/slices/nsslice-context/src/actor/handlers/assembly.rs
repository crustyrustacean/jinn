...
use nsslice_context_protocol::{AssemblyContext, CharRatioEstimator, estimate_entry_tokens};
use nullslop_actor::ActorContext;
use nullslop_protocol::context::{AssemblePrompt, PromptAssembled};
...
impl PromptAssemblyActor {
    ...
    pub(in crate::actor) fn ensure_strategy(&mut self, session_id: &SessionId) {
    ...
    pub(in crate::actor) async fn on_assemble_prompt(
        &mut self,
        cmd: &AssemblePrompt,
        ctx: &ActorContext,
    ) {
