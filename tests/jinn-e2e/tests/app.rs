...
use jinn_domain::FakeLlmServiceFactory;
use jinn_domain::InMemoryConfigStorage;
use jinn_domain::InMemoryUserPreferencesStorage;
use jinn_domain::LlmMessage;
use jinn_domain::LlmServiceFactoryService;
use jinn_domain::PinChatEntry;
use jinn_domain::PinPosition;
use jinn_domain::ProviderRegistry;
use jinn_domain::ProviderRegistryService;
use jinn_domain::ProvidersConfig;
use jinn_domain::SessionId;
use jinn_domain::StateReadGuard;
use jinn_domain::StreamCompleted;
use jinn_domain::StreamCompletedReason;
use jinn_domain::StreamToken;
...
use jinn_tui::selection::SelectionState;
...
impl AppWorld {
    ...
    fn new_app_world_impl(
        ...
        let app = TuiApp {
            ...
            which_key: WhichKeyInstance::new(jinn_tui::keymap::init(), Scope::Normal),
            suspend: Suspend::new(),
            event_thread: None,
    ...
    pub fn press_key(&mut self, code: KeyCode, modifiers: KeyModifiers) {
        ...
        self.app.handle_msg(jinn_tui::msg::Msg::Input(event));
    }

    /// Routes an intent through the app.
    pub fn route_intent(&mut self, intent: jinn_domain::Intent) {
        self.app.route_intent(intent);
    }

    /// Submits a command to the core's message channel.
    #[allow(dead_code)]
    pub fn submit_command(&self, cmd: jinn_domain::Command) {
...
/// Parses a human-readable mode name into [`jinn_domain::Mode`].
fn parse_mode(name: &str) -> jinn_domain::Mode {
    match name.to_lowercase().as_str() {
        "normal" => jinn_domain::Mode::Normal,
        "input" => jinn_domain::Mode::Input,
        "picker" => jinn_domain::Mode::Picker,
    ...
    let scope = match parse_mode(&mode) {
        ...
        jinn_domain::Mode::Input => {
            ...
            state
                .frontend
                .scope_stack
                .push(jinn_domain::common::app_state::FocusScope::Input);
            drop(state);
            Scope::Input
        }
        jinn_domain::Mode::Picker => Scope::PickerProvider,
    };
...
async fn when_enqueue_user_message(world: &mut AppWorld, text: String) {
    ...
    world.submit_command(jinn_domain::Command::EnqueueUserMessage(
        EnqueueUserMessage {
            session_id,
...
async fn when_submit_cancel_stream(world: &mut AppWorld) {
    let session_id = world.active_session_id();
    world.submit_command(jinn_domain::Command::CancelStream(CancelStream {
        session_id,
    }));
    world
        .wait_until(|s| {
            let phase = s.active_session().phase();
            phase != jinn_domain::PhaseKind::Streaming
        })
        .await;
}
...
async fn when_submit_tools_registered(world: &mut AppWorld, tool_name: String) {
    world.submit_event(Event::ToolsRegistered(jinn_domain::ToolsRegistered {
...
async fn when_submit_tool_execution_completed(world: &mut AppWorld) {
    ...
    world.submit_event(Event::ToolExecutionCompleted(
        jinn_domain::ToolExecutionCompleted {
            session_id,
            result: ToolResult {
...
async fn when_submit_pin_entry(world: &mut AppWorld, position: String) {
    ...
    drop(state);
    world.submit_command(jinn_domain::Command::PinChatEntry(PinChatEntry {
        session_id,
        entry_id: last_id,
        position: pin_pos,
    }));
...
async fn when_submit_unpin_entry(world: &mut AppWorld) {
    ...
    let last_id = history.last().expect("history has entries").id.clone();
    drop(state);
    world.submit_command(jinn_domain::Command::UnpinChatEntry(UnpinChatEntry {
        session_id,
        entry_id: last_id,
    }));
    // Give the actor system time to process.
...
fn run_headless_script(world: &mut AppWorld, content: &str) {
    ...
    let lines: Vec<Vec<jinn_domain::KeyEvent>> = content
        .lines()
        .map(|line| line.trim())
...
async fn when_session_saved_and_reloaded(world: &mut AppWorld) {
    ...
    world.submit_command(jinn_domain::Command::SessionLoadRequested(
        jinn_domain::SessionLoadRequested {
    ...
    world
        .wait_until(|state| {
            state
                ...
                .iter()
                .any(|e| {
                    matches!(&e.kind, jinn_domain::ChatEntryKind::System(t) if t.contains("Warning: working directory"))
                })
        })
        .await;
    let state = world.state();
