...
impl TuiWorld {
    ...
    fn new_app() -> Self {
        ...
        let config = jinn_domain::ProvidersConfig {
            providers: vec![],
            aliases: vec![],
        ...
        let services = jinn_domain::Services {
            ...
            provider_registry: jinn_domain::ProviderRegistryService::new(
                jinn_domain::ProviderRegistry::from_config(config).expect("test registry"),
            ),
    ...
    fn route_command(&mut self, cmd: jinn_domain::Command) {
        self.app.handle_msg(jinn_tui::msg::Msg::Command(cmd));
    }

    /// Routes an intent through the app.
    fn route_intent(&mut self, intent: jinn_domain::Intent) {
...
fn when_routes_push_chat_entry(world: &mut TuiWorld, source: String, text: String) {
    world.route_command(jinn_domain::Command::PushChatEntry(
        jinn_domain::PushChatEntry {
            session_id: jinn_domain::SessionId::new(),
            entry: jinn_domain::ChatEntry::actor(source, text),
        },
    ));
}
...
fn then_history_entry_is_user(world: &mut TuiWorld, index: u64, text: String) {
    ...
    assert_eq!(
        entry.kind,
        jinn_domain::ChatEntryKind::User {
            display: text.clone(),
            expanded: text
        },
...
fn then_history_entry_is_actor(world: &mut TuiWorld, index: u64, source: String, text: String) {
    ...
    assert_eq!(
        entry.kind,
        jinn_domain::ChatEntryKind::Actor { source, text },
