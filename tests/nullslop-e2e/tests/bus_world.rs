...
impl BusWorld {
    ...
    /// Creates a session store service backed by a temp directory.
    fn test_session_store() -> SessionStoreService {
        SessionStoreService::new(Arc::new(JsonlSessionStore::new_in(
...
fn given_app_in_mode(world: &mut BusWorld, mode: String) {
    ...
    world.state.mode = parsed;
    if parsed == npr::Mode::Picker {
        world.state.active_picker_kind = Some(npr::PickerKind::Provider);
        nullslop_component::provider_picker::handler::load_provider_picker_items(
            &world.services,
            &mut world.state,
        );
        world.state.provider_picker.reset();
    }
}
...
fn then_no_pending_commands(world: &mut BusWorld) {
    // Only check commands, not events. Session persistence and other subsystems
    // may emit events that get processed in the next cycle.
    assert!(
        !world.bus.has_pending_commands(),
        "expected no pending commands"
    );
}
...
fn when_actor_starting(world: &mut BusWorld, name: String) {
    world.submit_event_and_process(npr::Event::ActorStarting {
        payload: npr::actor::ActorStarting {
            name,
            description: None,
        },
    });
...
fn then_active_strategy_is(world: &mut BusWorld, expected: String) {
    ...
    let expected_id = npr::PromptStrategyId::new(&expected);
    assert_eq!(strategy, &expected_id, "active strategy mismatch");
}
...
// ---------------------------------------------------------------------------
// Step definitions — Strategy Picker (Phase 5)
// ---------------------------------------------------------------------------

#[cucumber::given(expr = "the app is in Picker mode for strategy selection")]
fn given_app_in_picker_strategy_mode(world: &mut BusWorld) {
    world.state.mode = npr::Mode::Picker;
    world.state.active_picker_kind = Some(npr::PickerKind::ContextAssembly);
    nullslop_component::context_strategy_picker::entries::load_strategy_picker_items(
        &world.services,
        &mut world.state,
    );
    world.state.context_strategy_picker.reset();
}

#[cucumber::given(expr = "the context strategy picker selection is {int}")]
fn given_context_strategy_picker_selection(world: &mut BusWorld, index: u64) {
    world
        .state
        .context_strategy_picker
        .set_selection(index as usize);
}

#[cucumber::when(expr = "I submit OpenPicker with kind {string}")]
fn when_submit_open_picker(world: &mut BusWorld, kind: String) {
    let picker_kind = match kind.as_str() {
        "Provider" => npr::PickerKind::Provider,
...
fn then_context_strategy_filter_should_be(world: &mut BusWorld, expected: String) {
    assert_eq!(
        world.state.context_strategy_picker.filter(),
        expected,
        "context strategy picker filter mismatch"
    );
}

#[cucumber::then(expr = "the default strategy should be {string}")]
fn then_default_strategy_is(world: &mut BusWorld, expected: String) {
    assert_eq!(
        world.state.default_strategy(),
        &npr::PromptStrategyId::new(&expected),
        "default strategy mismatch"
    );
}

#[cucumber::then(expr = "the active picker kind should be {string}")]
fn then_active_picker_kind_is(world: &mut BusWorld, expected: String) {
    let expected_kind = match expected.as_str() {
        "Provider" => npr::PickerKind::Provider,
        "ContextAssembly" => npr::PickerKind::ContextAssembly,
        _ => panic!("unknown picker kind: {expected}"),
    };
    assert_eq!(
        world.state.active_picker_kind,
        Some(expected_kind),
        "active picker kind mismatch"
    );
}
