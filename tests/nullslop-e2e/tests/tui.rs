...
// ---------------------------------------------------------------------------
// Step definitions — Session Fork
// ---------------------------------------------------------------------------

/// Populates the active session with user and assistant messages.
#[cucumber::given(expr = "the active session has {int} user messages and {int} assistant messages")]
fn given_session_has_messages(world: &mut TuiWorld, user_count: u64, assistant_count: u64) {
    let mut state = world.app.core.state.write();
    ...
    for i in 0..assistant_count {
        state
            .active_session_mut()
            .push_entry(nullslop_domain::ChatEntry::assistant(format!(
...
#[cucumber::when(expr = "the user opens the fork picker")]
fn when_user_opens_fork_picker(world: &mut TuiWorld) {
    world.route_intent(nullslop_domain::Intent::OpenPicker {
        kind: nullslop_domain::PickerKind::SessionFork,
    });
...
fn then_fork_picker_active(world: &mut TuiWorld) {
    ...
    assert_eq!(
        ...
        "expected SessionFork picker to be active"
    );
}

/// Confirms the currently selected entry in the fork picker.
#[cucumber::when(expr = "the user confirms the fork picker selection")]
fn when_user_confirms_fork(world: &mut TuiWorld) {
    world.route_intent(nullslop_domain::Intent::PickerConfirm);
}

/// Asserts the active session history has the expected number of entries.
#[cucumber::then(expr = "the active session should have {int} history entries")]
fn then_active_session_history_count(world: &mut TuiWorld, count: u64) {
    let actual = world.app.core.state.read().active_session().history().len();
    assert_eq!(
        actual, count as usize,
        "expected {count} history entries, got {actual}"
...
/// Stores the current active session ID for later comparison.
#[cucumber::given(expr = "the current session id is recorded")]
fn given_record_session_id(world: &mut TuiWorld) {
    ...
    let _ = world.app.core.state.read().session.active_session_id().clone();
}
