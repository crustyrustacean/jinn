...
impl CoreBpe {
    ...
    /// assert!(tokens.contains(&199999)); // <|endoftext|> token id
    /// ```
    #[must_use]
    ...
    /// ```
    /// let enc = tiktoken::get_encoding("o200k_base").unwrap();
    /// let bytes = enc.decode(&[24912, 2375]);
    ...
    /// ```
    /// let enc = tiktoken::get_encoding("o200k_base").unwrap();
    /// let text = enc.decode_to_string(&[24912, 2375]).unwrap();
    /// assert_eq!(text, "hello world");
    /// ```
    ...
    /// ```
    /// let enc = tiktoken::get_encoding("o200k_base").unwrap();
    /// assert_eq!(enc.num_special_tokens(), 2);
    /// ```
    #[inline]
    #[must_use]
